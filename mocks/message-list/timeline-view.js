...
function updateStats() {
  ...
  if (toolResponseTimes.length > 0) {
    const avg = toolResponseTimes.reduce((a, b) => a + b, 0) /
      toolResponseTimes.length;
    document.getElementById("avg-response").textContent = Math.round(avg) +
      "ms";
  }
}
...
function createTimelineEvent(messageData, previousMessage) {
  ...
  if (eventType === "PostToolUse" && previousMessage) {
    ...
    if (
      prevParsed.hook_event_name === "PreToolUse" &&
      prevParsed.tool_name === parsedMessage.tool_name
    ) {
      const responseTime = new Date(messageData.timestamp) -
  ...
  if (parsedMessage.hasOwnProperty("stop_hook_active")) {
    bodyContent += `
      ...
      parsedMessage.stop_hook_active ? "Active" : "Inactive"
    }</span>
            </div>
        `;
  }
        ...
        <div class="timeline-card">
            <div class="timeline-header">
                <div class="timeline-title">
                    ...
                    ${
    parsedMessage.tool_name
      ? `<span class="tool-name">${
        parsedMessage.tool_name.replace("mcp__", "")
      }</span>`
      : ""
  }
                </div>
...
function addMessage(messageData) {
  ...
  const parsedMessage = JSON.parse(messageData.message);
  if (
    activeFilter !== "all" && parsedMessage.hook_event_name !== activeFilter
  ) {
