...
function toggleAllFilters(checkbox) {
  const filterCheckboxes = document.querySelectorAll(
    ".dropdown-item .toggle-switch[data-filter]",
  );
...
function applyFilters() {
  ...
  eventContainers.forEach((container) => {
    ...
    const toolText = eventRow.querySelector(".col-tool").textContent
      .toLowerCase();
    const sessionText = eventRow.querySelector(".col-session").textContent
      .toLowerCase();

    // Check all filters
    const matchesEventFilter = activeFilters.has(eventType);
    const matchesToolFilter = !toolFilter || toolText.includes(toolFilter);
    const matchesSessionFilter = !sessionFilter ||
      sessionText.includes(sessionFilter);
...
function getToolInfo(parsedMessage) {
  ...
  if (
    parsedMessage.hook_event_name === "Stop" ||
    parsedMessage.hook_event_name === "SubagentStop"
  ) {
...
function createEventRow(messageData) {
  ...
  row.addEventListener("click", (e) => {
    ...
    document.querySelectorAll(".event-row-container.expanded").forEach(
      (container) => {
